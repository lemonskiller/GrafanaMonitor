...
const serviceNameFromHeaders = (headers, defaultService) => {
  const value = headers['x-service-name']
    || headers['x-client-service']
    || headers['x-caller-service']
    || headers['x-app-name'];
  return String(Array.isArray(value) ? value[0] : value || defaultService || 'unknown-service').trim() || 'unknown-service';
};
...
export class MetricsRegistry {
  ...
  recordRequest({ model = 'unknown', endpoint = 'other', clientService = 'unknown-service', status = 0, durationSeconds = 0, promptTokens = 0, generatedTokens = 0, error = false }) {
    const group = this.groupFor(model);
    increment(this.requests, JSON.stringify([model, endpoint, clientService, String(status), group]));
    increment(this.promptTokens, JSON.stringify([model, clientService, group]), Number(promptTokens) || 0);
    increment(this.generatedTokens, JSON.stringify([model, clientService, group]), Number(generatedTokens) || 0);
    if (error) increment(this.errors, JSON.stringify([model, endpoint, clientService, group]));

    const durationKey = JSON.stringify([model, endpoint, clientService, group]);
  ...
  render({ up = false, available = [], loaded = [] } = {}) {
    ...
    for (const [key, value] of this.requests) {
      const [model, endpoint, clientService, status, group] = JSON.parse(key);
      lines.push(`ollama_requests_total{${labels({ model, endpoint, client_service: clientService, status, alert_email_group: group })}} ${value}`);
    }
    ...
    for (const [key, value] of this.errors) {
      const [model, endpoint, clientService, group] = JSON.parse(key);
      lines.push(`ollama_errors_total{${labels({ model, endpoint, client_service: clientService, alert_email_group: group })}} ${value}`);
    }
    ...
    for (const [key, value] of this.promptTokens) {
      const [model, clientService, group] = JSON.parse(key);
      lines.push(`ollama_prompt_tokens_total{${labels({ model, client_service: clientService, alert_email_group: group })}} ${value}`);
    }
    ...
    for (const [key, value] of this.generatedTokens) {
      const [model, clientService, group] = JSON.parse(key);
      lines.push(`ollama_generated_tokens_total{${labels({ model, client_service: clientService, alert_email_group: group })}} ${value}`);
    }
    ...
    for (const key of tokenKeys) {
      ...
      lines.push(`ollama_tokens_total{${labels({ model, client_service: clientService, alert_email_group: group })}} ${total}`);
    }
    ...
    for (const [key, duration] of this.durations) {
      const [model, endpoint, clientService, group] = JSON.parse(key);
      for (const bucket of HISTOGRAM_BUCKETS) {
        lines.push(`ollama_request_duration_seconds_bucket{${labels({ model, endpoint, client_service: clientService, alert_email_group: group, le: bucket })}} ${duration.buckets.get(bucket) ?? 0}`);
      }
      lines.push(`ollama_request_duration_seconds_bucket{${labels({ model, endpoint, client_service: clientService, alert_email_group: group, le: '+Inf' })}} ${duration.count}`);
      lines.push(`ollama_request_duration_seconds_sum{${labels({ model, endpoint, client_service: clientService, alert_email_group: group })}} ${duration.sum}`);
      lines.push(`ollama_request_duration_seconds_count{${labels({ model, endpoint, client_service: clientService, alert_email_group: group })}} ${duration.count}`);
    }
    return `${lines.join('\n')}\n`;
  }
}

export const createOllamaMonitor = ({ upstreamUrl, models = [], defaultGroup = 'ollama-default', defaultClientService = 'unknown-service', registry = null }) => {
  ...
  return createServer(async (clientRequest, clientResponse) => {
    ...
    const clientService = serviceNameFromHeaders(clientRequest.headers, defaultClientService);
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const defaultClientService = process.env.OLLAMA_DEFAULT_CLIENT_SERVICE ?? 'open-webui';
  ...
  if (metricsHost && metricsPort && (metricsHost !== host || metricsPort !== port)) {
    const metricsServer = createOllamaMonitor({ upstreamUrl, defaultClientService, registry });
